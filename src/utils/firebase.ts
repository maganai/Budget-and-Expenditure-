...
export const initAuth = (
  ...
  // Try to load any cached token from storage just for session persistence if available
  // But wait, the skill says:
  ...
  return onAuthStateChanged(auth, async (user: User | null) => {
    if (user) {
      if (cachedAccessToken) {
        ...
        // Since we don't persist access token in storage, 
        // a page refresh will require the user to log in again or re-trigger signInWithPopup (or silent token refresh, but popup is fine).
...
// Start Google sign-in with a popup
